...
def cornersHeuristic(state, problem):
    ...
    for i in range(len(corner_lst)):
        if corner_lst[i] == False:
            undiscovered.append(corners[i])

    cost = 0
    while len(undiscovered) != 0:
        min_index, min_dist = find_closest(pos, undiscovered)
        cost += min_dist
        pos = undiscovered[min_index]
...
def find_closest(location, lst):
    ...
    for i in range(len(lst)):
        distance = util.manhattanDistance(location, lst[i])
